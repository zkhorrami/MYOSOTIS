...
FUNCTION BCcal,wavelength,flux,lambda,weight,AVstar,Rv,Teff,par2vega,EXTmodel,DrainearrLam,DrainearrK; sed, filter
...
;  bolconstant = -2.5*alog10(4*!pi*567.0373/(3.24078^2.*3.844))
  ...
  for i=1, (size(wavelength))[-1]-1 do begin
    ltemp=wavelength[i]*1.0e-4 ; don't forget to convert A to um for extinction calculation!
    if (EXTmodel eq 'Fmodel' and wavelength[i] ge 1250. and wavelength[i] le 33333.) then alamarr[i]=extinctions(ltemp,Avstar,Rv)
...
;    par2 += wavelength[i]*wavelength_weight[i]*(wavelength[i]-wavelength[i-1])  ;!!!! par2 will be calculating from Vega flux
  ...
  BCfilter = Mbolsun + bolconstant - 10.*alog10(teff)+2.5*alog10(par1/par2vega);+21.10
  RETURN, BCfilter
END